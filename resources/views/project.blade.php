...
@section('main')
    <div class="container mx-auto px-4 py-8 rtl" dir="rtl">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 mb-8 p-6">
            <div
                class="grid grid-cols-1 md:grid-cols-3 gap-6 divide-y md:divide-y-0 md:divide-x md:divide-x-reverse divide-gray-100">
                <div class="flex flex-col items-center justify-center text-center">
                    <span class="text-gray-400 text-sm mb-1">نوع المشروع</span>
                    <span class="text-lg font-bold text-gray-800">{{ $project->project_type }}</span>
                </div>
                <div class="flex flex-col items-center justify-center text-center pt-4 md:pt-0">
                    <span class="text-gray-400 text-sm mb-1">الحالة</span>
                    <span class="text-lg font-bold text-[#498e49]">{{ $project->status }}</span>
                </div>
                <div class="flex flex-col items-center justify-center text-center pt-4 md:pt-0">
                    <span class="text-gray-400 text-sm mb-1">الموقع</span>
                    <span class="text-lg font-bold text-gray-800">{{ $project->location ?? 'غير محدد' }}</span>
                </div>
            </div>
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach ($project->properties as $property)
                        <a href="{{ route('properties', $property->id) }}" class="block">
                            <div
                                class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all duration-300 group hover:-translate-y-1">
                                <div class="h-48 w-full overflow-hidden bg-gray-100">
                                    @if ($property->propertiesImages->count() > 0)
                                        <img src="{{ asset('storage/' . $property->propertiesImages->first()->url) }}"
                                            alt="{{ $property->name }}"
                                ...
                                <div class="p-6">
                                    <div class="flex justify-between items-start mb-4">
                                        <div>
                                            ...
                                            <span
                                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#498e49]/10 text-[#498e49]">
                                                {{ $property->type }}
                                            </span>
                                        </div>
                                        <div class="text-left">
                                            <span
                                                class="block text-xl font-bold text-[#498e49]">{{ number_format($property->price) }}</span>
